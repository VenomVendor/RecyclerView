...
/**
 * A {@link android.support.v7.widget.RecyclerView.LayoutManager} implementation which provides similar functionality to
...
public class LinearLayoutManager extends RecyclerView.LayoutManager {
    ...
    /**
     * While trying to find next view to focus, LinearLayoutManager will not try to scroll more than this factor times
     * the total space of the list. If layout is vertical, total space is the height minus padding, if layout is
    ...
    /**
     * Helper class that keeps temporary rendering state. It does not keep state after rendering is complete but we
     * still keep a reference to re-use the same object.
     */
     ...
     * {@link LinearLayoutManager} make those decisions. Based on {@link #mOrientation}, an implementation is lazily
    ...
    /**
     * This keeps the final value for how LayoutManager shouls start laying out views. It is calculated by checking
     * {@link #getReverseLayout()} and View's layout direction.
     * {@link #onLayoutChildren(RecyclerView.Recycler, RecyclerView.State)} is run.
     */
    private boolean mShouldReverseLayout = false;

    /**
     * Works the same way as {@link android.widget.AbsListView#setStackFromBottom(boolean)} and it supports both
     * orientations. see {@link android.widget.AbsListView#setStackFromBottom(boolean)}
     */
    private boolean mStackFromEnd = false;

    /**
     * When LayoutManager needs to scroll to a position, it sets this variable and requests a layout which will check
     * this variable and re-layout accordingly.
     */
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (getChildCount() > 0) {
            final boolean didLayoutFromEnd = mLastStackFromEnd ^ mShouldReverseLayout;
    ...
    /**
     * Calculates the view layout order. (e.g. from end to start or start to end) RTL layout support is applied
     * automatically. So if layout is RTL and {@link #getReverseLayout()} is {@code true}, elements will be laid out
     * starting from left.
     */
    ...
    /**
     * Used to reverse item traversal and layout order. This behaves similar to the layout change for RTL views. When
     * set to true, first item is rendered at the end of the UI, second item is render before it etc. For horizontal
     * layouts, it depends on the layout direction. When set to true, If {@link android.support.v7.widget.RecyclerView}
     * is LTR, than it will render from RTL, if {@link android.support.v7.widget.RecyclerView} is RTL, it will render
     * from LTR. If you are looking for the exact same behavior of
     * {@link android.widget.AbsListView#setStackFromBottom(boolean)}, use {@link #setStackFromEnd(boolean)}
     */
     ...
     * @param offset The distance (in pixels) between the start edge of the item view and start edge of the
     *            RecyclerView.
     ...
     * @param dt This can be used to add additional padding to the visible area. This is used to detect children that
     *            will go out of bounds after scrolling, without actually moving them.
     */
     ...
     * @param renderState Current render state. Right now, this object does not change but we may consider moving it out
     *            of this view so passing around as a parameter for now, rather than accessing {@link #mRenderState}
     ...
     * 
     * @param focusDirection One of {@link View#FOCUS_UP}, {@link View#FOCUS_DOWN}, {@link View#FOCUS_LEFT},
     *            {@link View#FOCUS_RIGHT}, {@link View#FOCUS_BACKWARD}, {@link View#FOCUS_FORWARD} or 0 for not
     *            applicable
     * @return {@link RenderState#LAYOUT_START} or {@link RenderState#LAYOUT_END} if focus direction is applicable to
     *         current state, {@link RenderState#INVALID_LAYOUT} otherwise.
     */
     ...
     * method.
     * 
     * @return The adapter position of the first visible item or {@link RecyclerView#NO_POSITION} if there aren't any
     *         visible items.
     ...
     * 
     * @return The adapter position of the first fully visible item or {@link RecyclerView#NO_POSITION} if there aren't
     *         any visible items.
     ...
     * method.
     * 
     * @return The adapter position of the last visible view or {@link RecyclerView#NO_POSITION} if there aren't any
     *         visible items.
     * @see #findLastCompletelyVisibleItemPosition()
     * @see #findFirstVisibleItemPosition()
     */
     ...
     * horizontal, it will only check the view's left and right edges.
     * 
     * @return The adapter position of the last fully visible view or {@link RecyclerView#NO_POSITION} if there aren't
     *         any visible items.
     * @see #findLastVisibleItemPosition()
     * @see #findFirstCompletelyVisibleItemPosition()
     */
    ...
    /**
     * Used for debugging. Validates that child views are laid out in correct order. This is important because rest of
     * the algorithm relies on this constraint. In default layout, child 0 should be closest to screen position 0 and
     * last child should be closest to position WIDTH or HEIGHT. In reverse layout, last child should be closes to
    ...
    private void validateChildOrder() {
        ...
        final int lastPos = getPosition(getChildAt(0));
        final int lastScreenLoc = mOrientationHelper.getDecoratedStart(getChildAt(0));
        if (mShouldReverseLayout) {
            for (int i = 1; i < getChildCount(); i++) {
                final View child = getChildAt(i);
                final int pos = getPosition(child);
                final int screenLoc = mOrientationHelper.getDecoratedStart(child);
                if (pos < lastPos) {
                    logChildren();
    ...
    private static class RenderState {
        ...
        /**
         * Defines the direction in which the data adapter is traversed. Should be {@link #ITEM_DIRECTION_HEAD} or
         * {@link #ITEM_DIRECTION_TAIL}
         */
        int mItemDirection;
        ...
        /**
         * Used when RenderState is constructed in a scrolling state. It should be set the amount of scrolling we can
         * make without creating a new view. Settings this is required for efficient view recycling.
         */
        int mScrollingOffset;

        /**
         * Used if you want to pre-layout items that are not yet visible. The difference with {@link #mAvailable} is
         * that, when recycling, distance rendered for {@link #mExtra} is not considered to avoid recycling visible
         * children.
         */
        int mExtra = 0;

        /**
         * When LLM needs to layout particular views, it sets this list in which case, RenderState will only return
         * views from this list and return null if it cannot find an item.
         */
        ...
        /**
         * Gets the view for the next element that we should render. Also updates current item index to the next item,
         * based on {@link #mItemDirection}
         * 
